...
export const projects: Project[] = [
...

  {
    ...
    body: {
      ...
      process: {
        ko: "세 개의 벽을 각각 대화형 기록, 애착 기반 동기, 정직한 집계로 넘기로 하고 앱을 홈(대화) · 내 데이터(장부) · 우리(소속감) 세 존으로 나눴습니다. 가장 많은 시간을 쓴 것은 기능을 더하는 일이 아니라 하지 않을 일의 목록을 쓰는 일이었습니다. 마이너스 부호는 쓰지 않고, 배출 기록은 숫자를 만들지 않으며, 내 감축량을 보여주고 그것이 자연에 어떤 효과를 주는지 바로 보여주도록 설계했습니다.",
        en: "I set out to cross those three walls with conversational logging, affection-based motivation and honest aggregation, and split the app into three zones — Home for conversation, My Data as a ledger, and Us for belonging. What took the most time was not adding features but writing the list of things the product may not do. No minus sign, no figure produced by an emission log, and a design in which you see your own reduction and, right beside it, what that reduction does for nature.",
      },
      aiWorkflow: {
